...
import { ArrowRight, Mail, Phone, MapPin, Sparkles, FileText } from 'lucide-react';
...
export function HeroSection() {
  return (
    <section id="about" className="pt-24 sm:pt-32 lg:pt-36 pb-16 sm:pb-24 relative overflow-hidden w-full">
      ...
      <div className="absolute top-10 left-1/4 w-[600px] lg:w-[900px] h-[350px] lg:h-[500px] bg-cyan-500/15 blur-[140px] pointer-events-none rounded-full" />
      <div className="absolute top-20 right-1/4 w-[500px] lg:w-[800px] h-[350px] lg:h-[450px] bg-indigo-600/15 blur-[150px] pointer-events-none rounded-full" />
      ...
      <div className="w-full max-w-[1800px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 relative z-10">
          ...
          <div className="lg:col-span-7 xl:col-span-7 space-y-5 sm:space-y-6 text-left">
            ...
            >
              {PERSONAL_INFO.availableFor.map((role) => (
                <span
                  key={role}
                  className="px-2.5 sm:px-3 py-1 rounded-lg text-[11px] sm:text-xs font-semibold bg-slate-900/90 text-slate-300 border border-slate-800 font-mono"
                >
                  {role}
                </span>
              ))}
            ...
            >
              ...
              <button
                ...
                className="w-full sm:w-auto px-5 py-3 rounded-xl text-xs sm:text-sm font-semibold text-slate-400 hover:text-white hover:bg-slate-800/80 transition-all flex items-center justify-center gap-2"
              >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mt-12 sm:mt-16 w-full">
          {PERSONAL_INFO.stats.map((stat) => (
            <Card3D key={stat.label} glowColor="rgba(56, 189, 248, 0.25)" className="text-center p-4 sm:p-6">
